...
const targetSchema = z.object({ kind: z.enum(["department", "project"]), conversationId: z.uuid() });
...
export async function sendConversationMessageAction(
  ...
  const target = targetSchema.safeParse({ kind: formData.get("kind"), conversationId: formData.get("conversationId") });
  ...
  try {
    ...
    if (target.data.kind === "department") {
      await store.sendDepartment(actor, target.data.conversationId, formData.get("content"));
    } else {
