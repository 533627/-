...
import { TaskSubtaskList } from "@/features/tasks/task-subtask-list";
...
export default async function TasksPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="module-page space-y-6">
    <header className="module-header"><div className="flex flex-wrap items-end justify-between gap-4"><div><p>发布、执行和完成情况集中管理</p><h1 className="mt-2">任务待办</h1><p className="mt-3 max-w-2xl leading-7 text-base-content/70">{user.role === "EMPLOYEE" ? "逐条完成主任务中的小任务，全部确认后主任务会自动完成。" : user.role === "OPERATIONS_ADMIN" ? "可设置任务执行周期，并将工作拆成多条可确认的小任务。" : "跟进你负责执行或派发的任务，及时处理逾期事项。"}</p></div>{assignmentOptions ? <TaskPublishDialog projects={assignmentOptions.map((project) => ({ id: project.id, name: project.name, members: project.members.map((member) => ({ id: member.id, name: member.name, departmentName: member.department?.name ?? "未分配部门" })) }))} standaloneMembers={directAssignmentMembers.map((member) => ({ id: member.id, name: member.name, departmentName: member.department?.name ?? "未分配部门" }))} yesterdayTasks={yesterdayTaskTemplates.map((task) => ({ id: task.id, projectId: task.projectId ?? undefined, projectName: task.project?.name ?? "日常任务", assigneeId: task.assigneeId, assigneeName: task.assignee.name, title: task.title, description: task.description, priority: task.priority, startsAt: formatChinaDateTimeLocal(task.startsAt && task.startsAt > now ? task.startsAt : now), dueAt: formatChinaDateTimeLocal(nextReusableDueAt(task.dueAt, now)), subtasks: task.subtasks }))} /> : null}</div></header>
    ...
    {tasks.length ? <ul className="space-y-4">{tasks.map((task) => <li className="card card-border bg-base-100" data-testid={`task-${task.id}`} id={`task-${task.id}`} key={task.id}><div className="card-body gap-4 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="text-sm sm:text-right"><p className={task.isOverdue ? "font-semibold text-error" : "text-base-content/65"}>{task.startsAt ? `${formatTaskDate(task.startsAt)} 至 ` : "截止 "}{formatTaskDate(task.dueAt)}</p><p className="mt-1 text-base-content/50">版本 {task.version}</p></div>
      </div>
      ...
      <TaskSubtaskList isAssignee={task.assigneeId === user.id} projectId={task.projectId} subtasks={task.subtasks} taskId={task.id} />
      ...
      <TaskActionPanel canReview={user.role === "SUPER_ADMIN" || task.assignedById === user.id} hasSubtasks={task.subtasks.length > 0} isAssignee={task.assigneeId === user.id} projectId={task.projectId} status={task.status} taskId={task.id} version={task.version} />
