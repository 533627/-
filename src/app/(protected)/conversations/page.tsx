...
export default async function ConversationsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const fallback = groups.departments[0]
    ? { kind: "department" as const, id: groups.departments[0].id }
    : groups.projects[0]
      ? { kind: "project" as const, id: groups.projects[0].id }
      : null;
  const selected = requestedId ? { kind: requestedKind, id: requestedId } : fallback;
  let conversation = null;
  if (selected) {
    try { conversation = selected.kind === "project" ? await store.getProject(actor, selected.id) : await store.getDepartment(actor, selected.id); }
  ...
  return <div className="space-y-5">
    ...
    <div className="grid min-h-[38rem] overflow-hidden rounded-box border border-base-300 bg-base-100 lg:grid-cols-[18rem_minmax(0,1fr)]">
      <aside className="border-b border-base-300 bg-base-200/50 p-3 lg:border-r lg:border-b-0" aria-label="群聊列表">
        <GroupSection title="部门群">{groups.departments.map((department) => <GroupLink active={conversation?.kind === "department" && conversation.id === department.id} href={`/conversations?kind=department&id=${department.id}`} key={department.id} label={department.name} meta="部门群" />)}</GroupSection>
      ...
      {conversation ? <section className="flex min-h-[38rem] min-w-0 flex-col" aria-labelledby="conversation-title">
        ...
        <MessageComposer conversationId={conversation.id} kind={conversation.kind} />
